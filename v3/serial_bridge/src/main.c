/*
 * Controller Application Main - Simplified for Serial Bridge
 * This version keeps your existing functionality and just logs received data
 * The Python serial bridge will capture this data and send it to the web dashboard
 */
...
static void udp_receive_cb(void *aContext, otMessage *aMessage,
  ...
  // Log the received message - this will be captured by the serial bridge
...
void send_multicast_command(const char *cmd) {
  ...
  if (otUdpOpen(instance, &udpSocket, NULL, NULL) == OT_ERROR_NONE) {
    ...
    LOG_INF("Sent multicast command: %s", cmd);
  } else {
...
void button_pressed(const struct device *dev, struct gpio_callback *cb,
  ...
  if (streaming) {
    ...
    LOG_INF("Streaming started - button pressed");
    send_multicast_command(CMD_START);
  } else {
    gpio_pin_set_dt(&led, 0);
    printk("Streaming stopped\n");
    LOG_INF("Streaming stopped - button pressed");
...
void print_network_status(void) {
  ...
  LOG_INF("Network Status: %s", role_str);
  
  if (role != OT_DEVICE_ROLE_DISABLED && role != OT_DEVICE_ROLE_DETACHED) {
    uint16_t panId = otLinkGetPanId(instance);
    uint8_t channel = otLinkGetChannel(instance);
    LOG_INF("PAN ID: 0x%04x, Channel: %d", panId, channel);
  }
}

/* Network status monitoring timer */
static void network_status_timer(struct k_timer *timer_id) {
  print_network_status();
}

K_TIMER_DEFINE(network_timer, network_status_timer, NULL);
...
int main(void) {
  ...
  // Start network status monitoring (every 30 seconds)
  k_timer_start(&network_timer, K_SECONDS(10), K_SECONDS(30));

  // Print initial network status
  print_network_status();

  LOG_INF("Controller ready. Press button to start/stop streaming.");
